...
- In this repo, Codex is the primary operator/runtime target. Prefer the Codex-first path over speculative multi-runtime compatibility work.
...
- **Project-scoped state** -- prefer repo-local state roots such as `.wave/codex/`, `.wave/state/`, and `.wave/traces/` when the task references runtime artifacts.
...
- **Operator shell bias** -- when the task touches the TUI or dashboard, treat the right-side panel as the built-in status surface and keep it backed by authoritative Wave state.
