...
- The current compatibility boundary is now narrower: planning, queue, and control/operator truth already flow through reducer-backed projections over compatibility run inputs, while structured result envelopes are not yet authoritative and proof lifecycle plus replay ratification still depend on compatibility surfaces under `.wave/state/runs/` and `.wave/traces/runs/`.
- The next executable cutover is Wave 12: land structured result envelopes and proof lifecycle so closure input stops depending directly on free-form marker scanning while legacy adapters remain explicit and replay ratification stays later work.
