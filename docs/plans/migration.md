...
- Wave 13 is now the landed scheduler-authority cutover: claim and lease truth are canonical, local claim admission is exclusive, and live leases renew and expire in canonical scheduler state while execution remains serial.
- Wave 14 is now the natural point for true parallel-wave execution, worktree isolation, merge discipline, fairness, and protected closure capacity.
- Wave 15 should land runtime policy and multi-runtime adapters, Wave 16 should make code/doc/component-state mismatches durable contradiction state, Wave 17 should add portfolio and release acceptance state, and Wave 19 should make the planner emit architecture intent, invariants, and staged gate plans as part of the authored contract.
