...
- Wave 15 is dependency-complete here through an explicit manual-close override after a failed promotion-conflict run, and Wave 16 is now the latest canonically completed wave in this workspace after closure-only rerun reconciliation fixed the authority merge for carried-forward agent success.
...
- Operators can directly inspect run, agent, queue, and control truth from the shell without switching to a separate CLI status path first, and they can act on queue selection, rerun intents, and manual-close recovery in-place.
...
- True intra-wave MAS execution is still target-state, not live: agents inside one wave still run serially against one shared wave-local filesystem view. The next architecture step for graph-aware intra-wave concurrency, per-agent sandboxes, merge queue authority, and invalidation routing is documented in `docs/implementation/true-multi-agent-wave-architecture.md` and `docs/plans/true-multi-agent-wave-rollout.md`.
...
- Later waves may assume `wave adhoc` and `wave delivery` exist in this worktree. They must not assume `wave dep`, live-host deployment proof, or true intra-wave MAS until those slices are explicitly landed.
