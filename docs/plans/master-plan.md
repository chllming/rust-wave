...
- The next forward architecture wave is Wave `17`: move above the landed design-authority boundary into portfolio, release, and acceptance-package semantics while keeping the docs and operator/runtime boundary explicit. Wave `13` and Wave `14` still need authority-state reconciliation in this workspace, but that is cleanup against already-landed code slices rather than the next new architecture boundary.
...
- The next implementation-oriented architecture wave is Wave `17`: the portfolio, release, and acceptance-package layer above the now-landed design-authority boundary, while keeping the repo-local envelope-first proof boundary honest.
...
1. Wave `17`: land the portfolio, release, and acceptance-package layer above waves. Initiatives, milestones, release trains, outcome contracts, rollout readiness, ship decisions, known risks, and outstanding debt should become first-class delivery truth instead of summary prose. Live proof for this wave should show one initiative or release object aggregating multiple waves into a coherent ship or no-ship state.
2. After Wave `17`, cut `factory-mas-v2`: make true intra-wave multi-agent execution a first-class release on top of the product-factory baseline. That release should add opt-in MAS wave graphs, per-agent sandboxes, merge queue authority, invalidation routing, and reducer-backed ready-set selection without breaking the current serial default.
3. After the MAS release boundary is stable, add a dedicated TUI and control-plane ergonomics wave that implements `docs/implementation/design.md` directly: keyboard model, operator action lifecycle, blocker triage, per-agent live status, orchestrator approvals, proof drill-down, and multi-wave concurrency visibility should become first-class UX work rather than incidental fallout from backend slices.
