...
- Fold control-discipline hardening into later waves without reopening the landed scheduler-authority slice: Wave `14` owns true parallel execution plus worktree isolation, Wave `15` owns runtime policy and multi-runtime adapters, Wave `16` owns durable design-authority workflow state, Wave `19` owns planner-emitted invariants and staged gate plans, and later waves carry the delivery/package layer plus non-authoritative telemetry.
...
- Treat execution isolation as a hard architectural requirement for the live parallel-wave boundary: one worktree per active parallel wave and no shared root-workspace mutation across parallel implementation waves. The later intra-wave MAS step may add per-agent sandboxes inside that admitted wave.
- Keep the next intra-wave MAS step explicit: after the product-factory delivery layer merges, move from shared per-wave execution to graph-aware per-agent sandboxes, merge queue authority, and invalidation routing without regressing to launcher-owned truth.

## Landed Baseline

- The code slices through Wave `16` are present in the current worktree, but the live authority state is not a clean linear `0..16` completion chain in this workspace:
  - Waves `0` through `12` are canonically completed here.
  - Wave `13` scheduler-authority code is landed, but the current workspace still shows Wave `13` ready and claimable with no canonical latest run.
  - Wave `14` parallel-wave/worktree code is landed and proof-backed, but the current workspace only has a dry-run Wave `14` proof run and it remains blocked on Wave `13`.
  - Wave `15` is dependency-complete here via explicit manual-close override after a failed promotion-conflict run.
  - Wave `16` is canonically completed here after the control-plane latest-run reconciliation for closure-only reruns.
  - Within that authority-state caveat, the landed code slices are:
  ...
  - Wave `16` lands durable question, assumption, decision, contradiction, human-input, dependency-handshake, and selective invalidation semantics above the runtime boundary.
...
- The current worktree also seeds the product-factory layer above waves: `wave delivery ...` surfaces, `docs/plans/delivery-catalog.json`, repo-local `wave adhoc ...`, design gates, delivery soft-state overlays, and shell-friendly status or watch wrappers are now part of the branch baseline.
- This shared-plan landing does not claim Wave `12` cont-QA closure; that final verdict still belongs to `A0`.
- Wave `17` now owns the delivery layer above waves: portfolio, release, and acceptance-package semantics sit above the landed design-authority boundary while keeping the docs and operator/runtime boundary explicit. Wave `13` and Wave `14` still need authority-state reconciliation in this workspace, but that is cleanup against already-landed code slices rather than the next new architecture boundary.
...
- The current parallel-wave baseline should assume wave-level execution isolation: each active parallel wave owns its own worktree, while agents inside that wave share the same wave-local filesystem view until the later MAS sandbox cutover lands.
- Wave `18` is now partially landed in the current tree: MAS-authored-wave contract changes, durable orchestrator and MAS control records, MAS-aware CLI/app-server/TUI surfaces, a per-agent-sandbox runtime path with concurrent safe launch, runtime-backed operator/autonomous head control, directive-delivery transport semantics, and reducer-backed recovery state are all present. The remaining closure gap is the live proof run.
...
1. Wave `18`: close the first true MAS pilot. The remaining Wave 18 work is one live proof run showing at least two agents running concurrently inside one wave plus durable steering, targeted recovery, operator reclaim, and honest closure.
2. Wave `19`: land planner-emitted invariants and staged gate plans once the MAS runtime substrate is live. The planner should be able to emit machine-usable gate plans, graph invariants, and approval rules instead of relying on handwritten operator interpretation.
3. After Wave `19`, deepen the operator and head ergonomics directly against `docs/implementation/design.md`: keyboard model, operator action lifecycle, blocker triage, proof drill-down, and multi-wave plus multi-agent concurrency visibility should become first-class UX work rather than incidental fallout from backend slices.
4. After that ergonomics wave, continue with richer replay parity, telemetry, benchmarks, and any remaining package-parity work, but only once the scheduler, runtime, delivery, isolation, and operator-ergonomics foundations above are proven in repo-local use.
