...
- Waves `0` through `13` are now code-landed in the current worktree:
...
- The next executable work on paper is Wave `14`: move from serial scheduler enforcement to true parallel-wave execution and per-wave worktree isolation while keeping the docs and architecture boundary explicit.
...
- The next implementation-oriented architecture wave is Wave `14`: true parallel-wave execution, worktree isolation, merge discipline, fairness, and protected closure capacity while keeping the repo-local envelope-first proof boundary honest.
...
1. Wave `14`: land true parallel-wave execution, worktree management, merge discipline, fairness, preemption policy, and protected closure capacity. This is where the harness stops being a serial queue runner operationally, not just architecturally. Live proof for this wave should show two non-conflicting waves running concurrently in separate worktrees, merge or integration results captured explicitly, and scheduler fairness or closure-capacity behavior visible in projections.
2. Wave `15`: land runtime policy and runtime plurality on the new execution boundary. Codex and Claude should become sibling adapters behind one executor API, skill projection should become late-bound, and runtime selection or fallback should move into explicit policy instead of launcher folklore. Live proof for this wave should show the same wave contract launching through Codex and a Claude path or fixture-backed equivalent while preserving the same reducer or projection semantics.
3. Wave `16`: land question, assumption, decision, contradiction, human-input, dependency-handshake, and invalidation semantics as durable workflow state. This is where design-first loops stop leaking back into prose. Live proof for this wave should show an unresolved question or superseded decision reopening or blocking the correct downstream wave and invalidating the right proofs without broad manual judgment.
4. Wave `17`: land the portfolio, release, and acceptance-package layer above waves. Initiatives, milestones, release trains, outcome contracts, rollout readiness, ship decisions, known risks, and outstanding debt should become first-class delivery truth instead of summary prose. Live proof for this wave should show one initiative or release object aggregating multiple waves into a coherent ship or no-ship state.
5. After Wave `17`, add a dedicated TUI and control-plane ergonomics wave that implements `docs/implementation/design.md` directly: keyboard model, operator action lifecycle, blocker triage, per-agent live status, orchestrator approvals, proof drill-down, and multi-wave concurrency visibility should become first-class UX work rather than incidental fallout from backend slices.
6. After that UX wave, continue with richer replay parity, planner-emitted invariants and staged gate plans, telemetry, benchmarks, and any remaining package-parity work, but only once the scheduler, runtime, delivery, isolation, and operator-ergonomics foundations above are proven in repo-local use.
7. As waves execute, correct any gap between the shared-plan story, the code, the live-proof artifacts, and the operator-visible runtime state before promoting any later evidence.
