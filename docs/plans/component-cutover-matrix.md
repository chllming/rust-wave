...
| `operator-status-tabs` | `repo-landed` | `Overview`, `Agents`, `Queue`, `Proof`, and `Control` tabs are live, keyboard navigable, and share the same reducer-backed queue/proof/control truth as the CLI surfaces, including in-place operator actions. Later waves can rely on these as the primary operator affordances for queue selection, proof review, and rerun intent. |
