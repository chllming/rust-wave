...
| Ad-hoc plan, run, and promote | `wave adhoc plan|run|show|promote` | repo-local product-factory branch now ships `plan|run|list|show|promote` over `.wave/state/adhoc/` with promotion back into `waves/` | `post-cutover` | keep the repo-local adhoc lane on the same domain model and continue unifying planner-owned packets later | `17+` |
