...
depends_on = [11]
validation = ["cargo test -p wave-results -p wave-gates -p wave-runtime --locked", "cargo test -p wave-app-server -p wave-cli -p wave-trace --locked", "cargo run -p wave-cli -- doctor --json", "cargo run -p wave-cli -- control proof show --wave 12 --json"]
